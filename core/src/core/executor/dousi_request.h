...
struct DousiRequest {
    ...
    DousiRequest(uint32_t object_id, uint64_t stream_id, std::shared_ptr<char> buffer_ptr, const size_t &buffer_size, std::string method_name)
            : object_id_(object_id),
            stream_id_(stream_id),
            buffer_ptr_(std::move(buffer_ptr)),
